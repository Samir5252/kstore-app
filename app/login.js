...
// --- (Todos tus componentes de Iconos y CustomAlert van aquí, sin cambios) ---
...
export default function LoginScreen() {
  ...
  // ... (Toda la lógica de Google Auth y otras funciones se mantienen igual)
  ...
  const handleEmailChange = (text) => {
    const newEmail = text;
    const shouldShowPassword = newEmail.length > 0;
    
    // Si el campo de contraseña va a aparecer por primera vez, activamos la animación.
    if (shouldShowPassword && !isPasswordVisible) {
          ...
          <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
            ...
            <View style={styles.linksContainer}>
                <TouchableOpacity style={{ marginBottom: 15 }} onPress={() => router.push('/register')}>
                    <Text style={styles.linkText}>¿No tienes cuenta? <Text style={{fontWeight: 'bold'}}>Crea una</Text></Text>
                </TouchableOpacity>
            ...
            <TouchableOpacity 
              style={[styles.googleButton, (!isGoogleAvailable || isLoading) && styles.googleButtonDisabled]} 
              disabled={isLoading}
              onPress={handleGooglePress}
            >
              <GoogleIcon />
              <Text style={[styles.googleButtonText, (!isGoogleAvailable || isLoading) && styles.googleButtonTextDisabled]}>
...
const styles = StyleSheet.create({
    ...
    inputContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'white', borderRadius: 12, paddingHorizontal: 15, marginBottom: 15, height: 55, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 5, elevation: 3 },
    ...
    linksContainer: { flexDirection: 'column', alignItems: 'center', marginTop: 20 },
